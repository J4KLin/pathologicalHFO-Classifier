...
fullnn_shelve_path = SAVE_DIR / 'featnn_fullmdl.out'
lounn_shelve_path = SAVE_DIR / 'featnn_loumdls.out'
...
temp = ~np.isin(feature_cols,ignorefeats)
feature_cols = feature_cols[temp].tolist()
...
fig.suptitle("HFO Feat (BIDS) BOTH vs REST NN")
...
for pid in pids:
    ...
    train,valid = train_test_split(df.iloc[curidxs,:].reset_index(drop=True),
...
plt.title("Leave Patient Out Neural Net.\n ROC " + data_type,fontsize = 24)
